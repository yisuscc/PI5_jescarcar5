...
public class TestsPI5 {
	...
	public static <V, E> GraphPath<V, E> testAStar(EGraph<V, E> graph, GraphPath<V, E> gp) {
		var alg_star = gp == null ? AStar.of(graph) : AStar.of(graph, null, gp.getWeight(), gp);
	...
	public static <V, E> GraphPath<V, E> testPDR(EGraph<V, E> graph, GraphPath<V, E> gp) {
		var alg_pdr = gp == null ? DPR.of(graph) : DPR.of(graph, null, gp.getWeight(), gp, true);
	...
	public static <V, E, S> GraphPath<V, E> testBT(EGraph<V, E> graph, GraphPath<V, E> gp) {
		var alg_bt = gp == null ? BT.of(graph) : BT.of(graph, null, gp.getWeight(), gp, true);
	...
	private static <V, E> void toDot(String type, Graph<V, E> g, GraphPath<V, E> sol) {
		...
		GraphColors.toDot(g, "grafos/" + type + "-" + file + num_test + ".gv", V::toString, E::toString,
				v -> GraphColors.styleIf(Style.bold, vs.test(v)), e -> GraphColors.styleIf(Style.bold, es.test(e)));
	}
