...
public record CafeVertex(Integer index, List<Integer> remaining)
	...
	public static Integer limiteVarCafe(Integer index, List<Integer> rem) {
		/*
		 * Dados un remaining y un index devuelve cual es el maxímo numeno de Kilos que
		 * se puede coger de cada variedad
		 */

		List<Integer> rem2 = List2.copy(rem);
		Map<String, Double> comp = DatosEjercicio1.getVariCafe().get(index).composicion();// si el map fuese initeger
																							// integer
		// seria mas sencillo
